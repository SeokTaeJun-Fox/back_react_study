...
// 사용자가 구매할 상품을 입력한다.
  ...
  const store = {
   ...
   const [buyListShow, setBuyListShow] = useState([]);

   const [price, setPrice] = useState(0);
   const [total, setTotal] = useState(0);

   const [income, setIncome] = useState(store.income);
      ...
      if(e.key === "Enter")
      {
        ...
        if(productList.some(({name}) => inputName === name)){
          ...
          if(income < tmpPrice){
            ...
            setIncome(income - tmpPrice)
            console.log(income - tmpPrice);
          }
        }
          ...
          setBuyListShow(newArray);

          setTotal(buyList.reduce((acc, cur) => acc + cur["price"], 0));
        }
      }
  }
  ...
  return (
    <div>
         <h1>결과 출력:</h1>
         <input type="text" placeholder='구매할 상품을 입력하세요' onKeyUp={handleOnKeyUp}/>
         <p>상품 가격: {price}</p>
