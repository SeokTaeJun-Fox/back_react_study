...
const AttendanceContainer = () => {
  ...
  const attendanceList = attendanceData.map(({id, name, isPresent}) => {
    return (
      <Attendance key={id} name={name} isPresent={isPresent} />
    );
  });

  let totalMoney = attendanceData
    .filter(({isPresent}) => isPresent)
    .map(({money}) => money)
    .reduce((acc, curr) => acc += curr, 0);
  // attendanceData.forEach(({money, isPresent}) => {
  //   if(isPresent)
  //     totalMoney += money;
  // });
  ...
  return (
    <div>
      <ol>
        ...
        <AttendanceMoney money={totalMoney} />
    </div>
  );
};
